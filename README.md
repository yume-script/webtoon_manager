...
을 BookOasis 카테고리탭 플러그인으로 이식. **버전 1.6.7**
...
- 🟡 밀린 회차가 있는데 전부 실패/유료로 하나도 못 받았을 때 "UP" 뱃지가
  안 뜨던 문제를 고쳤습니다.
- 🟡 구독중 카드에 **"지금 다운로드"** 버튼을 추가했습니다(회차를 직접 고르지
...
- ✅ 신규 회차 자동 다운로드(작품별 상한 + 휴식 시간), 구독중 카드의 "수동 다운로드"
  버튼(자동 다운로드 대신 "수동 다운로드" 탭으로 이동 + 자동 조회), 카드의 "다시 확인"
  버튼(파일을 직접 지운 회차를 다음 다운로드 때 다시 잡게 함)
