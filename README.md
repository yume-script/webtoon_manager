...
## Playwright 폴백(선택, 무거움)

일부 회차는 이미지가 정적 HTML에 없고 페이지가 열린 뒤 자바스크립트로 나중에
불러와지는 방식이라, 기본(HTTP 요청 + 정규식) 방식으로는 못 잡습니다. 이런
회차를 위해 실제 헤드리스 브라우저(Chromium)로 페이지를 열어 로드되는 이미지
요청을 가로채는 폴백을 넣었습니다. 기본은 **꺼짐**이며, 설정에서
"이미지 추출 실패 시 Playwright 폴백 사용"을 켜야 동작합니다.

사용하려면 컨테이너 안에서 아래를 한 번 실행해 Chromium을 설치해야 합니다
(pip 설치만으로는 브라우저 바이너리까지 받아지지 않습니다):
...
Chromium이 설치되어 있지 않으면 폴백은 조용히 비활성 상태로 남고(첫 실패 시
로그에 안내만 남김), 기본 방식 결과만 사용합니다 — 파이프라인 자체가 죽지는
않습니다. 브라우저를 회차마다 새로 띄우면 너무 느려서, 한 번의 스캔/다운로드
사이클(또는 수동 다운로드 1회 실행) 동안은 브라우저 인스턴스를 재사용하고
끝나면 정리합니다.
