...
def _loop(get_cfg_func, run_full_cycle_func):
...
def ensure_started(get_cfg_func, run_full_cycle_func):
    ...
    with _lock:
        ...
        if _pid_alive(existing_pid) and existing_pid != str(os.getpid()):
            ...
            return False
        _write_lock()
        t = threading.Thread(target=_loop, args=(get_cfg_func, run_full_cycle_func),
